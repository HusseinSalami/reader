/**
 * Tests for Excel Export Handler
 * 
 * Tests the GET /exams/:examId/export/excel endpoint
 */
...
const { mockExportToExcel, mockGetExam } = vi.hoisted(() => ({
...
import { handler } from './export-excel';

describe('Export Excel Handler', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  const createEvent = (
    ...
    httpMethod: 'GET',
    path: `/exams/${examId}/export/excel`,
  ...
  describe('Successful Export', () => {
    it('should generate Excel export and return pre-signed URL', async () => {
      ...
      const mockExportResult: ExportResult = {
        exportId: 'export-789',
        downloadUrl: 'https://s3.amazonaws.com/bucket/export-789.xlsx?signature=abc',
      ...
      mockExportToExcel.mockResolvedValue(mockExportResult);
      ...
      expect(mockExportToExcel).toHaveBeenCalledWith(examId, customerId);
      
      const body = JSON.parse(result.body);
      expect(body.message).toBe('Excel export generated successfully');
      expect(body.export).toEqual(mockExportResult);
      expect(body.export.downloadUrl).toContain('.xlsx');
    ...
    it('should include record count in response', async () => {
      ...
      mockExportToExcel.mockResolvedValue({
        exportId: 'export-789',
        downloadUrl: 'https://s3.amazonaws.com/bucket/export-789.xlsx',
  ...
  describe('Error Handling', () => {
    it('should return 500 when export service fails', async () => {
      ...
      mockGetExam.mockResolvedValue({ examId, customerId } as Exam);
      mockExportToExcel.mockRejectedValue(new Error('Excel generation failed'));
      ...
      const body = JSON.parse(result.body);
      expect(body.error).toContain('Failed to generate Excel export');
      expect(body.message).toContain('Excel generation failed');
    });
