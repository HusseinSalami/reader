...
import { FileCode, Plus, Edit2, Trash2, Loader, X, Sparkles } from 'lucide-react';
...
export default function Templates() {
  ...
  const [initialDocTypeId, setInitialDocTypeId] = useState<string | undefined>(undefined);
  const [editingTemplate, setEditingTemplate] = useState<Template | null>(null);

  useEffect(() => {
    loadData();
    
    // Check URL parameters to auto-open AI generator
    const urlParams = new URLSearchParams(window.location.search);
    const shouldGenerate = urlParams.get('generate') === 'true';
    const docTypeId = urlParams.get('docType');
    
    if (shouldGenerate && docTypeId) {
      console.log('Auto-opening AI generator for document type:', docTypeId);
      setInitialDocTypeId(docTypeId);
      // Wait for data to load, then open AI generator
      setTimeout(() => {
  ...
  const handleCreateFromExample = (example: typeof EXAMPLE_TEMPLATES[0]) => {
  ...
  const handleSaveTemplate = async (data: any) => {
    try {
      const payload = {
        ...
        rules: data.rules.map((rule: any, idx: number) => ({
      ...
      if (data.templateId) {
        await templateApi.update(data.templateId, payload);
      } else {
      ...
      throw new Error(err.response?.data?.error?.message || 'Failed to save template');
    }
  };

  const handleAITemplateGenerated = async (generatedTemplate: any) => {
    // Open the editor with the AI-generated template
    setEditingTemplate({
      templateId: '',
      name: generatedTemplate.name,
      description: generatedTemplate.description,
      documentTypeId: generatedTemplate.documentTypeId,
      customerId: '',
      rules: generatedTemplate.rules,
      aiEnhanced: generatedTemplate.aiEnhanced,
      createdAt: '',
      updatedAt: '',
    } as any);
      ...
      {showEditor && (
        <TemplateEditor
          ...
          initialData={editingTemplate ? {
            ...
            documentTypeId: editingTemplate.documentTypeId,
            templateId: editingTemplate.templateId,
            rules: editingTemplate.rules,
            aiEnhanced: editingTemplate.aiEnhanced,
          } : undefined}
